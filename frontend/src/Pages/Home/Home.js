...
function Home() {
  ...
  const fetchSuggestions = async (input, type) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/suggestion/suggestions?input=${input}`
      );
  ...
  const getlistdataapi = async (e) => {
    ...
    if (RL.length !== 0) {
      const formattedList = RL.map((item) => ({
        userId: item.user?._id?.toString() || "",
        name: item.user?.name || "",
        phone: item.user?.phone?.toString() || "",
        isVerified: item.user?.isVerified?.toString() || "",
        scord: item.src?.coordinates
          ? { lat: item.src.coordinates[1], lng: item.src.coordinates[0] }
          : { lat: null, lng: null },
        dcord: item.dest?.coordinates
          ? { lat: item.dest.coordinates[1], lng: item.dest.coordinates[0] }
          : { lat: null, lng: null },
      }));
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/list/deletelist",
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="container">
        <form>
          <div className="form-group">
            ...
            {activeInput === "source" &&
              (sourceSuggestions.length > 0 || loadingSuggestions) && (
        ...
        <div>
          ...
          {searching &&
            listdata.map((user) => (
              <div
                className={`container ribbonbody ${
